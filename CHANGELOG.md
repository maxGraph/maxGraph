...
- `EdgeHandler.isHandleVisible()` now uses `EdgeStyleRegistry.allowsIntermediateHandles()` instead of checking against the `EdgeStyle.EntityRelation` function reference.
  ...
  In particular, if you register `EdgeStyle.EntityRelation` yourself (e.g. when using `BaseGraph`), you must include `{ allowIntermediateHandles: false }` in the metadata to preserve the previous behavior. The simplest way to do so is to call the new `registerEntityRelationEdgeStyle()` function, which sets the correct metadata for you.
