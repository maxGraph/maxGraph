...
#### `packages/website/docs/usage/extending.md` (NEW)

The reference material for both halves of this subject is scattered today: the type-level half exists only in
`CHANGELOG.md:37-58` and in the compile check `packages/ts-support/src/module-augmentation.ts`, and no website page
mentions custom style properties at all (grepping `website/docs/` for `augment` returns nothing). This page is where
both halves meet.

- Front matter following the local convention: `sidebar_position: 10`, which is what every page except `graph`,
  `global-configuration` and `migrate-from-mxgraph` uses, so the page sorts alphabetically among its peers, plus a
  one-line `description`. The sidebar is autogenerated (`packages/website/sidebars.ts:14`) and the category label
  comes from `usage/_category_.json`, so there is no wiring to do.
- Section 1, extending `CellStateStyle` by module augmentation. The `declare module '@maxgraph/core'` block, the fact
  that `CellStyle extends CellStateStyle` so the property is visible on both, and the minimum TypeScript version
  (raised to 3.9 by `67bdd9c9e`). This documents a feature that shipped one release ago with no user-facing page.
- Section 2, THE KEY INSIGHT for the reader, stated before the mechanics: a custom property of any other type needs
  nothing at all. The decoders coerce by the shape of the VALUE, so a number property already decodes as a number and
  a string as a string. Booleans are the single exception, because `"1"` is indistinguishable from a number by shape
  alone, which is why they and only they need to be declared.
- Section 3, declaring custom boolean properties for the codecs:
  `registerCustomBooleanCellStylePropertiesForCodecs(...)` once at setup, next to the codec registration. State that
  one call covers all three decode shapes (`style="myFlag=1"`, `<Object myFlag="1" as="style"/>` and
  `<add as="myFlag" value="1"/>` in a stylesheet), that it is DECODE-ONLY because the encoder decides from the value's
  type rather than its key so a custom boolean already exports as 1/0, that it is global like every other maxGraph
  `register*`, and that `unregisterAllCustomBooleanCellStylePropertiesForCodecs()` exists for test teardown and
  clears only the custom set.
- Section 4, the per-codec alternative: overriding `isBooleanValueAttribute` on a custom codec, for an application
  that wants the behavior scoped to its own codec rather than globally.
- Section 5, the limits. Whatever is decided for `ObjectCodec.decodeChild` (open decision 3) must be stated here
  rather than discovered by a reader. Cross-link rather than duplicate: `codecs.md` for the codec model,
  `global-configuration.md` for the other registries, `plugins.md` and `tree-shaking.md` for the neighbouring
  extension points.
- Scope note for the future: the page is deliberately about extending the STYLE types for now. The generic title
  leaves room for other extension subjects, which is why sections 4 and 5 cross-link instead of absorbing them.
...
- Add a link to the new Extending guide from the "Using custom object and custom Codec" section (`:162-177`), which is
  the natural entry point, and a second link next to the corrected `:212` sentence for a reader who arrives there
  from a search. Keep the examples themselves on the guide, so the two pages do not drift.
...
- Its `## Styles` section (`:53-105`) already lists the five style registries and their unregister functions. Add the
  new register and unregister pair there for discoverability, linking to the Extending guide for the example rather
  than repeating it.
