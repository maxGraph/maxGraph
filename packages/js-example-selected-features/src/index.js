...
const initializeGraph = (container) => {
  ...
  // Register shapes
  // The RectangleShape not registered here because it is always available by default. It is the fallback shape when no shape is returned
  ...
  StyleRegistry.putValue('rectanglePerimeter', Perimeter.RectanglePerimeter); // default
  StyleRegistry.putValue('orthogonalEdgeStyle', EdgeStyle.OrthConnector);
  // TODO register edge markers when available in maxGraph
...
document.getElementById('fit-center').addEventListener('click', () => {
  graph.getPlugin('fit')?.fitCenter({ margin: 20 });
});
