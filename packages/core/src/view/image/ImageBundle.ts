 ...
 * or the fallback should be returned in {@link getImage}.
 *
...
class ImageBundle {
   ...
   * or fallback, depending on {@link alt}. The fallback is returned if
   * {@link alt} is `true`, the value is returned otherwise.
   */
  getImage(key: string) {
    let result = null;
