...
function createGraphAndExport(moduleType, outputDirectoryName) {
...
<!-- Generated with maxGraph ${constants.VERSION} by js-example-nodejs using ${moduleType} on ${generationDate} -->`;
...
${modelAsXml}`;
  ...
  const outputDirectory = `build/${outputDirectoryName}`;
  ...
  class SimpleSvgExporter {
    ...
    export(options) {
      ...
      // we are sure that we have an SVG node here, it is always created at Graph creation
      // Notice that for now, the SVG is not pretty printed, so it is not indented
      const svgNode = this.graph.container.querySelector('svg');
...
exports.createGraphAndExport = createGraphAndExport;
