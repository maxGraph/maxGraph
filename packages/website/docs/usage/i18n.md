...
Skipping built-in translations helps reduce bundle size and improve [tree-shaking](./tree-shaking.md) during the build process.
:::
...
`maxGraph` includes built-in translations for the following languages, covering [`Graph`](./graph.md) and `Editor`:
- Chinese
- English
- French
- German
...
- **Live demo**: [HelloPort](https://maxgraph.github.io/maxGraph/demo/?path=/story/connections-helloport--default)
