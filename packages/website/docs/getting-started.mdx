...
:::tip
This example uses `Graph`, which loads all built-in shapes, styles and plugins so that you can get started without configuration. For a production application, use [`BaseGraph`](./usage/graph.md#basegraph) instead and register only the features you need. See the [Tree-Shaking](./usage/tree-shaking.md) page.
:::
