...
:::tip
This example uses `Graph`, which registers all built-in shapes and styles and loads the default plugins, so that you can get started without configuration. For a production application, use [`BaseGraph`](./usage/graph.md#basegraph) instead and register only the features you need. See the [Tree-Shaking](./usage/tree-shaking.md) page.
:::
